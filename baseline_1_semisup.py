...
class SETIDataset_unlbl(Dataset):
    def __init__(self, df, transform=None, conf=None):
        self.df = df.reset_index(drop=True)
        self.labels = df['target'].values
        self.dir_names = df['dir'].values
    ...
    def __getitem__(self, idx):
        ...
        image = torch.from_numpy(image).unsqueeze(dim=0)
        image_aug = torch.from_numpy(image_aug).unsqueeze(dim=0)
...
class SETIDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit':
            ...
            self.train_dataset_unlbl = SETIDataset_unlbl(pseudo_df, transform=train_transform,conf=self.conf)
    ...
    def train_dataloader(self):
        train_dataloader = DataLoader(self.train_dataset, batch_size=int(self.conf.batch_size/2), num_workers=4, shuffle=True, pin_memory=True, drop_last=True)
        train_dataloader_unlbl = DataLoader(self.train_dataset_unlbl, batch_size=int(self.conf.batch_size/2), num_workers=4, shuffle=True, pin_memory=True, drop_last=True)
