...
class L2ConstraintedNet(nn.Module):
    def __init__(self, org_model, alpha=16):
        super().__init__()
        self.org_model = org_model
        self.alpha = alpha

    def forward(self, x):
        x = self.org_model.global_pool(self.org_model.forward_features(x)).flatten(1)
        l2 = torch.sqrt((x**2).sum()) 
        x = self.alpha * (x / l2) 
        x = self.org_model.classifier(x)
        return x
...
class LitSystem(pl.LightningModule):
    def __init__(self, conf):
        ...
        self.model = timm.create_model(model_name=self.hparams.model_name, num_classes=2, pretrained=True, in_chans=2,
                                       drop_rate=self.hparams.drop_rate, drop_path_rate=self.hparams.drop_path_rate)
        self.model = L2ConstraintedNet(self.model)
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_hat = self.model(x)
        loss = lam * self.criteria(y_hat, y.flatten()) + (1 - lam) * self.criteria(y_hat, y[index].flatten())
    ...
    def validation_step(self, batch, batch_idx):
        x, y = batch
        y_hat = self.model(x)
        loss = self.criteria(y_hat, y.flatten())
