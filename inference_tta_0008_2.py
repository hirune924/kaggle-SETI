...
def inference(models, test_loader):
    ...
    probs = [[] for i in models]
    with torch.no_grad():
      for i, (images) in tk0:
          images = images[0].cuda()
          tta_preds = [[] for i in models]
          for idx, model in enumerate(models):
              ...
              #tta_preds[idx].append(y_preds.to('cpu').numpy())
          #avg_preds = np.mean(avg_preds, axis=0)
          
              probs[idx].append(y_preds.to('cpu').numpy())
      for idx, _ in enumerate(models):
          probs[idx] = np.concatenate(probs[idx])
          probs[idx] = rankdata(probs[idx], method='average')/ rankdata(probs[idx], method='average').max()
