...
class SETIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_aaa = np.vstack(image[[0,2,4]]).transpose((1, 0))
        image_bcd = np.vstack(image[[1,3,5]]).transpose((1, 0))
        
        img_pl = Image.fromarray(image_aaa).resize((self.conf.height, self.conf.width), resample=Image.BICUBIC)
        image_aaa = np.array(img_pl)

        if self.transform is not None:
            image_aaa = self.transform(image=image_aaa)['image']
        image_aaa = torch.from_numpy(image_aaa).unsqueeze(dim=0)

        img_pl = Image.fromarray(image_bcd).resize((self.conf.height, self.conf.width), resample=Image.BICUBIC)
        image_bcd = np.array(img_pl)

        if self.transform is not None:
            image_bcd = self.transform(image=image_bcd)['image']
        image_bcd = torch.from_numpy(image_bcd).unsqueeze(dim=0)
        
        label = torch.tensor([self.labels[idx]]).float()
        return image_aaa, image_bcd, label
...
def inference(models, test_loader):
    ...
    with torch.no_grad():
      for i, (images) in tk0:
          images_aaa = images[0].cuda()
          images_bcd = images[0].cuda()
          avg_preds = []
          for model in models:
              y_preds = model(images_aaa)/2.0
              y_preds += model(vflip(images_aaa))/2.0
              y_preds -= model(images_bcd)/2.0
