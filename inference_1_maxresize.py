...
class SETIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_pl = Image.fromarray(image).resize((512, 1638), resample=Image.BICUBIC)
        image = np.array(img_pl)
        image = F.adaptive_max_pool2d(torch.Tensor(image).unsqueeze(0), output_size=(512,512)).numpy()[0]
